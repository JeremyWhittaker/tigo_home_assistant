# Tigo Energy Cloud v0.1.2 quality checklist

This is the implementation and release acceptance record for v0.1.2. Update
...
| A-11 | Topology loads at setup, caches for 24 hours, and preserves identity across reordering/change. | implemented | Topology refresh plus dynamic entity/metadata reconciliation tests. |
...
| A-13 | Every discovered CCA is polled and per-module power is merged by newest valid source sample. | implemented | `test_snapshot_merges_newest_valid_power_from_every_cca`. |
...
| H-06 | Coordinator preserves last valid context while exposing fetch health and continuously advancing source freshness independently. | implemented | Failure-age/staleness progression, update-failure entity, and recovery tests. |
...
| H-09 | Every discovered module device exposes enabled power and daily-energy entities, including modules added by a later topology refresh. | implemented | Dynamic topology entity-addition test plus live 44 devices/88 module entities. |
...
| H-11 | Power/energy units, device classes, state classes, precision, source timestamps, and availability match `docs/DATA_MODEL.md`. | implemented | Entity state-metadata and independent power/energy timestamp tests plus live-state inspection. |
...
| H-15 | No automatic historical Recorder backfill is performed in v0.1.2. | implemented | No Recorder writes in source; README and dashboard guide document the expected initial history gap. |
